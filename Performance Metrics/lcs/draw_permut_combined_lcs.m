fig_title = 'longest-common-sequence-hidden';
fname1 = 'lcs_gamma_combined_sp';
...
jitter = [-0.2, -0.1, 0.1, 0.2]
color = {'#ff860d', '#00a933', '#55308d', '#dd0000'}

for i = 2 : length(modelNames)
    fname = ['perf_' modelNames{i}];
    load([fname '.mat']);
    
    perf = 100*perf_lcs_ratio; % convert to %
...
perf = 100*perf_lcs_ratio(:,1); % convert to %
%% get confidence intervals from boostrap and draw
Nm = size(perf,2);
[ci m]= bootci(10000,@mean,perf);
